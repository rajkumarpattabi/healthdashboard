...
**Version:** 1.6 — BUILD-READY (revised section names & display order)
**Date:** 09-Aug-2026
...
**Status:** Ready to build on your go. Mockup v5 (matching this spec, incl. revised sections) delivered alongside.
...
- **v1.6 — section names & order (your review):** sections renamed to standard report names and given a fixed display order (§5.4): **Blood Glucose → Insulin → Lipid Profile → Kidney Profile → Complete Blood Count → Thyroid Profile → Vitamins**. "Diabetic"→"Blood Glucose" (per your steer; "Glycemic Profile" is the more clinical alternative), "Renal"→"Kidney Profile", "Haematology"→"Complete Blood Count", plus "…Profile" suffixes; Insulin placed immediately after Blood Glucose.
...
|---|---|---|---|
| Hemoglobin | Complete Blood Count | g/dL | "Haemoglobin (HB)", "Haemoglobin", "Hemoglobin" |
| RBC Count | Complete Blood Count | 10^6/µL | "RBC (Red Blood Cell Count)", "RBC Count" (Million/cmm ↔ 10^12/L) |
| WBC Count (Total) | Complete Blood Count | 10^3/µL | "Total WBC Count", "WBC count, Total" |
| Platelet Count | Complete Blood Count | 10^3/µL | "Platelet Count" (Lakhs/cumm ↔ 10^9/L) |
| HbA1c | Blood Glucose | % | "HbA1c", "Glycosylated Hb (HbA1C)", "Glycosylated Hemoglobin(GHb/HbA1c)" |
| Fasting Glucose | Blood Glucose | mg/dL | "GLUCOSE FASTING (FBS)", "Glucose-Blood-Fasting" |
| Fasting Insulin | Insulin | µIU/mL | "Insulin Fasting", "Fasting Insulin" |
| Creatinine | Kidney Profile | mg/dL | "CREATININE", "*Creatinine" |
| Urea | Kidney Profile | mg/dL | "UREA", "*Urea" |
| Uric Acid | Kidney Profile | mg/dL | "URIC ACID", "Uric Acid" |
| Total Cholesterol | Lipid Profile | mg/dL | "Total Cholesterol" |
| HDL Cholesterol | Lipid Profile | mg/dL | "HDL Cholesterol" |
| LDL Cholesterol | Lipid Profile | mg/dL | "LDL Cholesterol" |
| Triglycerides | Lipid Profile | mg/dL | "Triglycerides" |
| TSH | Thyroid Profile | µIU/mL | "Thyroid Stimulating Hormone (TSH)" |
...
### 5.4 Section taxonomy & display order
Every parameter maps to exactly one canonical **section**. Sections use standard report names and render in this fixed order (empty sections hidden):

1. **Blood Glucose** — HbA1c, Fasting Glucose *(clinical alternative name: "Glycemic Profile")*
2. **Insulin** — Fasting Insulin, C-Peptide *(placed immediately after Blood Glucose)*
3. **Lipid Profile** — Total Cholesterol, HDL, LDL, Triglycerides, VLDL, ratios
4. **Kidney Profile** — Creatinine, Urea/BUN, Uric Acid *(a.k.a. Renal Profile / RFT)*
5. **Complete Blood Count** — Hemoglobin, RBC, WBC + differential, Platelets, indices *(the CBC; "Haematology" is the lab department, not the panel)*
6. **Thyroid Profile** — TSH, T3, T4 (as available)
7. **Vitamins** — Vitamin D, Vitamin B12 (extend to "Vitamins & Minerals" if minerals appear)

Any parameter whose section isn't yet mapped is placed in a trailing **"Other"** section rather than dropped, and flagged for dictionary review (ties to FR-B/AC-B3).
...
- **AC-E1.** Parameters grouped under `section`, each a labelled block, shown in the fixed order defined in §5.4 (Blood Glucose, Insulin, Lipid Profile, Kidney Profile, Complete Blood Count, Thyroid Profile, Vitamins). Empty sections are hidden.
...
*End of SRS v1.6 (build-ready; revised section taxonomy/order plus report picker, navigation, Sync tab, and sparse-parameter handling). On your go I'll build: (1) the two CSVs from all 11 reports, validated against §11, then (2) the mealfast-style PWA with the four-tab layout, report picker, sparse-aware trends, and Drive sync.*
